...
import com.E_Commerce_Web_App.pages.CartPage_Freq; 
...
public class CartPageTest_Freq extends BaseTest { 
@Test 
...
home.clickProducts(); 
ProductPage_Freq product = new ProductPage_Freq(driver); 
CartPage_Freq cart = new CartPage_Freq(driver); 
...
test.pass("Deleted first product from cart") .addScreenCaptureFromPath(ScreenshotUtilities.capturescreen(driver, "DeleteFirstProduct")); 
...
cart.clickCheckout(); 
test.pass("Clicked Proceed To Checkout") .addScreenCaptureFromPath(ScreenshotUtilities.capturescreen(driver, "Checkout")); 
} 
}